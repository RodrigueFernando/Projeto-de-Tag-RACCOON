...
cart_products.forEach((currentProduct) => {
   
    if(currentProduct.product_stock_quantity > 0){
        var duplicated = cart_products_sanitized.findIndex(id =>{
            return currentProduct.product_id == id.product_id;
        })>-1;
        if(!duplicated) {
            cart_products_sanitized.push(currentProduct);
        }
        
    }
    currentProduct.product_price =parseFloat(currentProduct.product_price)
    currentProduct.product_stock_quantity =parseInt(currentProduct.product_stock_quantity);
});
