...
cart_products.forEach((element) => {    
    element.product_price = Number(parseFloat(element.product_price).toFixed(2));
    element.product_quantity = Number(element.product_quantity);
});

function getCalcula(){
     totalPrice = cart_products.reduce((preVel, element)=>{
        return preVel + element.product_price * element.product_quantity;
    },0);
    var freeShipping = (totalPrice > 300) ? true: false
    return{
        totalPrice,
        freeShipping
    };
...
var cartPricesInfo = getCalcula();
console.log("cartPrincesInfo = ",cartPricesInfo);
